...
void StaticData::load(const char *filename) {
	...
	for (uint32 i = 0; i < rectListsCount; i++) {
		...
		for (uint32 itemIndex = 0; itemIndex < itemCount; itemIndex++) {
			RectItem rectItem;
			rectItem.rect.x1 = UPSCALE_X(fd.readUint16LE());
			rectItem.rect.y1 = UPSCALE_Y(fd.readUint16LE());
			rectItem.rect.x2 = UPSCALE_X(fd.readUint16LE() + 1) - 1;
			rectItem.rect.y2 = UPSCALE_Y(fd.readUint16LE() + 1) - 1;
			...
			for (uint32 subItemIndex = 0; subItemIndex < subItemCount; subItemIndex++) {
				...
				subRectItem.rect.x1 = UPSCALE_X(fd.readUint16LE());
				subRectItem.rect.y1 = UPSCALE_Y(fd.readUint16LE());
				subRectItem.rect.x2 = UPSCALE_X(fd.readUint16LE() + 1) - 1;
				subRectItem.rect.y2 = UPSCALE_Y(fd.readUint16LE() + 1) - 1;
	...
	for (uint32 i = 0; i < hitRectListsCount; i++) {
		...
		for (uint32 itemIndex = 0; itemIndex < itemCount; itemIndex++) {
			HitRect hitRect;
			hitRect.rect.x1 = UPSCALE_X(fd.readUint16LE());
			hitRect.rect.y1 = UPSCALE_Y(fd.readUint16LE());
			hitRect.rect.x2 = UPSCALE_X(fd.readUint16LE() + 1) - 1;
			hitRect.rect.y2 = UPSCALE_Y(fd.readUint16LE() + 1) - 1;
