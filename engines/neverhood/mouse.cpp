...
void Mouse::init(uint32 fileHash) {
	...
	_x = _vm->getMouseX();
	_y = _vm->getMouseY();
	createSurface(2000, UPSCALE(32, 32));
	SetUpdateHandler(&Mouse::update);
	SetMessageHandler(&Mouse::handleMessage);
	_drawOffset.x = 0;
	_drawOffset.y = 0;
	_drawOffset.width = UPSCALE_X(32);
	_drawOffset.height = UPSCALE_Y(32);
